...
const float minGew=0.010; //mindeste belastung die ein klebstoff aushalten muss 
...
int maxBelastung[sens] = {5,    //maximale belastung eines sensors; schutz der Sensoren vor überlastung
                            50,
                            200,
                            0,
                            0};
...
void setup() {
  ...
  int calibration_factorTemp=calibration_factor[sensorType];
  calibration_factor[sensorType];
  LCDSimul("Ready!");
  LCDSimul("n");
  lcd.clear();
  lcd.setCursor(0, 0);
...
void menu0() {
  ...
  for(int i=0;i<(6-(((String)tmpWeight).length()));i++){
    lcd.print(" ");
  }
  lcd.setCursor(10, 2);
...
void Belastungstest() {
  ...
  if (belastungstest==4){//Stufe 4 des Belastungstests, auf Bruch oder Sensorüberlastung warten
    ...
    if(maxiBelastung>=minGew){ //abreißen prüfen
      if((maxiBelastung-tmpWeight)>=(minGew*10)){
        belastungstest=6;
      }
    }
  }
