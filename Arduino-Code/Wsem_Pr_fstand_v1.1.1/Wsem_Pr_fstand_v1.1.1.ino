/*
   ...
   Autor: Paul Sänger
   Neuere Versionen unter Umständen unter https://github.com/padiyul/Klebstoffpruefstand/releases
...
void setup() {
  ...
  if(digitalRead(dip1)==HIGH) SerialExchange=true; //Serielle Einstellungen prüfen
  ...
  Serial.begin(19200);// Übertragungsgeschwindigkeit an den PC (Standard: 9600; Schnellstes: 2000000, passend: 19200)
  lcd.init();
  lcd.backlight();
  ...
  lcd.setCursor(3, 3);
  lcd.print(version);
  sensorType=readEEPROM(disk1, 0);                                                //Daten von EEPROM lesen
...
void menu2EventOkLong() {
  switch(sensorType){   //EEPROM speichern
    case 0:
...
void menu0() { //Menü anzeigen
  if (menuOnce != 0) { //Menümaske
    lcd.clear();
    lcd.setCursor(0, 0);
    lcd.print("Waage");
    lcd.setCursor(19, 0);
  ...
  lcd.setCursor(9, 1);        //sich ändernde Werte
  ...
  LCDSimul(F("Waage     0")); //Output für betrieb ohne LCD (Debugging)
  LCDSimul("n");
...
void motorBlink(){    //Blinken der kontoll-LED auf der Relaiskarte
...
void motor() { //dirakt richtung des Motors; 1=zug aufbauen, -1=zug ablassen, 0=motor stoppen
  ...
  switch(zustand){ //DEA zur detaillierten Steuerung des Motors
    case 0:
...
void Belastungstest() {
  ...
  if (belastungstest==4){//Stufe 4 des Belastungstests, auf Bruch oder Sensorüberlastung warten
    ...
    if(maxiBelastung>=minGew){ //abreißen prüfen
      if((maxiBelastung-tmpWeight)>=((maxiBelastung*0.5)+minGew)){
        belastungstest=6;
      }
    }
  }
...
void BelastungstestAbbruch(){ //Aufrufen um Belastungstest ab zu brechen z.B. durch Usereingabe
  belastungstest=7;
}
...
void SerialSend() { //Sendet Seriell Informationen des Prüfstandes an den PC
  if (SerialExchange) {
    ...
    Serial.println("; ");
    /*Serial.print("  ");                 //mögliche Erweiterung, mehr Informationen am PC
...
byte shifting(byte input, int isol) { //isol: zu isolierender Bit
  byte tmp = input << (8 - isol);   //Zu isolierender Bit wird zuerst an den linken Rand des Bytes geschoben, alle werte links davon gehen verloren
  tmp = tmp >> 7;                   //Zu isolierender Bit wird an den rechten Rand des Bytes geschoben, alles rechts des bits geht verloren
  return tmp;                       //alleiniger Bit, entweder 1 oder 0, wird zurück gegeben
}

void save(){ //Startet Speicheranimation
  arrowState=1;
}
...
void writeEEPROM(int deviceaddress, unsigned int eeaddress, byte data ){ //byte auf EEPROM schreiben
 ...
 union u_tag{ //union zur umwandlung von float zu 4 Byte und umgekehrt zum speichern und lesen aus dem EEPROM
    byte bin[4];
    float num;
  }u;
  
void writeFloatEEPROM(int deviceaddress, unsigned int eeaddress, float data ){ //float in EEPROM schhreiben mit Hilfe von union
    u.num=data;
    Wire.beginTransmission(deviceaddress);
    Wire.write((int)(eeaddress >> 8));   // MSB
    Wire.write((int)(eeaddress & 0xFF)); // LSB
...
float readFloatEEPROM(int deviceaddress, unsigned int eeaddress){ //float aus EEPROM lesen
...
byte readEEPROM(int deviceaddress, unsigned int eeaddress ){ //byte aus EEPROM lesen
  byte rdata = 0xFF;
...
void Debug() {    //wenn aktiviert Serielle Debugdaten ausgeben
  if (debug) {
    if (Serial.available()) {
      char temp = Serial.read();
      if (temp == 'd')
...
void Debug(String in) {  //wenn aktiviert Serielle Debugdaten ausgeben
  if (debug) {
    Serial.println(in);
  }
}

void LCDSimul(String in) { //wenn aktiviert Serielle LCD-Simulation ausgeben
